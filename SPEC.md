...
7. 会话、模型、检索、Tool 调用的结构化日志；MVP 使用 SQLite 会话上下文，生产化需进一步接入租户隔离、过期清理和备份策略。
...
- 当前 MVP 不承诺跨进程、跨重启恢复会话；会话上下文仅用于演示和固定评测。
...
- 固定评测集当前为 55 条，覆盖中文口语、省略式追问、连续多轮会话、支付敏感独立路由、业务边界、Tool 异常、知识无依据和规则版本。
