...
当前 POC 不包含真实认证、真实订单/物流连接、真实退款、实时质量运营和多租户隔离。会话、工单和退货申请已使用 SQLite 持久化；事件指标仍为进程内存储，进程重启后会清空。规则检索链路已具备生产级 Provider 接口，但默认仍使用本地确定性 embedding/reranker，生产必须配置真实模型服务和索引治理。
