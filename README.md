...
1. 从“演示导览”点击“开始体验”，消费者询问物流，系统返回订单状态、最新节点、地点和预计到达时间。
2. 再体验退货判断，系统根据订单和生效规则判断资格；确认后才创建“待审核”退货申请。
3. 最后体验投诉转人工，观察系统停止自动处置、创建工单并生成接管摘要。
4. 切换人工客服或客服主管，查看工单处理和质量结果。
...
- 规则问答：通过向量召回、关键词召回、RRF 融合和 reranker 重排检索本地版本化规则文档，回答附标题和版本；本地评测使用确定性 Provider，尚未接入真实生产模型服务。
...
多轮会话当前支持：通过 `session_id` 继承已绑定订单和上一轮意图；首轮缺少退货原因时，下一轮可补充原因；物流查询后可追问预计到达时间；同一会话连续两次未解决时自动转人工。会话上下文已使用 SQLite 持久化并按用户隔离；生产环境仍需补充租户隔离、过期策略、迁移、备份和脱敏治理。
...
当前 POC 不包含真实认证、真实订单/物流连接、真实退款、实时质量运营和多租户隔离。会话、工单、退货申请和质量事件均使用 SQLite 持久化，服务重启后仍可查询；事件默认存储于 `runtime/events.db`，可通过 `EVENTS_DB_PATH` 配置。规则检索链路已具备生产级 Provider 接口，但默认仍使用本地确定性 embedding/reranker，生产必须配置真实模型服务和索引治理。
